<x-app-layout>
  ...
  <script>
    const deleteChirp = ({
      ...
      const confirmDelete = confirm('Delete the Chirp?');
      if (confirmDelete) {
        const chirpDeleteForm = target.closest('form');
        chirpDeleteForm.submit();
      }
    }
  </script>
            ...
            <x-dropdown>
              ...
              <x-slot name="content">
                ...
                <form method="POST" action="{{ route('chirps.destroy', $chirp) }}" id="deleteForm">
                  @csrf
                  @method('delete')
                  <x-dropdown-link :href="route('chirps.destroy', $chirp)" onclick="event.preventDefault(); deleteChirp(event);">
                    {{ __('Delete') }}
                  </x-dropdown-link>
                </form>
